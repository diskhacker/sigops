...
const mod = (await import("./agent-tools.routes.js")).default;
...
describe("agent-tools routes", () => {
  it("lists", async () => {
    ...
    expect((await app.request("/r?agentId=x&toolName=x")).status).toBe(200);
  });
  ...
  it("create", async () => {
    ...
    const res = await app.request("/r", {
      ...
      body: JSON.stringify({agentId: "x", toolName: "x"}),
    });
