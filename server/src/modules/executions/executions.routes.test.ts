...
const mod = (await import("./executions.routes.js")).default;
...
describe("executions routes", () => {
  it("lists", async () => {
    ...
    expect((await app.request("/r?workflowId=x&status=PENDING")).status).toBe(200);
  });
  ...
  it("create", async () => {
    ...
    const res = await app.request("/r", {
      ...
      body: JSON.stringify({workflowId: "x", triggerType: "x"}),
    });
